...
class StoryCrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, value=None, mask=None, transformer_options=None, **kwargs):
    ...
    def new_forward(
        ...
        if write:
            ...
            self.id_bank[cur_step] = [
                cache_hidden_states[:, img_ind, indices[img_ind], :].reshape(cache_hidden_states.shape[0], -1, channel).clone()
        ...
        if cur_step <1:
            ...
            if random_number > rand_num:
                ...
                if write:
                    ...
                    hidden_states = hidden_states.reshape(-1,img_nums,nums_token,channel)
                    encoder_arr = [
                        hidden_states[:, img_ind, indices[img_ind], :].reshape(hidden_states.shape[0], -1, channel)
                        for img_ind in range(img_nums)
                    ]
                    for img_ind in range(img_nums):
